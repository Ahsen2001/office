...
@section('content')
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-8">
            <div class="d-flex justify-content-end gap-2 h-100 align-items-center">
                ...
                <a href="{{ route('staff.appointments.create') }}" class="btn btn-primary">Book Appointment</a>
            </div>
        </div>
    </div>
    ...
    <form method="GET" class="card soft-card mb-4"><div class="card-body row g-3 align-items-end">
        ...
        <div class="col-md-3"><label class="form-label">Department</label><select name="department_id" class="form-select"><option value="">All departments</option>@foreach($departments as $department)<option value="{{ $department->id }}" @selected($departmentId == $department->id)>{{ $department->name }}</option>@endforeach</select></div>
    ...
    <div class="card soft-card"><div class="table-responsive">
        <table class="table align-middle mb-0">
            <thead class="table-light"><tr><th>No</th><th>Person</th><th>Department</th><th>Date/Time</th><th>Status</th><th>Officer</th><th class="text-end">Actions</th></tr></thead>
            <tbody>
                @forelse($appointments as $appointment)
                    <tr>
                        ...
                        <td class="text-end"><a href="{{ route('staff.appointments.show', $appointment) }}" class="btn btn-sm btn-outline-primary">View</a><a href="{{ route('staff.appointments.edit', $appointment) }}" class="btn btn-sm btn-outline-secondary">Edit</a></td>
                    </tr>
                @empty
